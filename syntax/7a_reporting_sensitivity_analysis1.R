# R file for:
# Sensitivity analysis 1: weighted glm + vocvHC(type = "HC1")
# 
# For paper:
...
# Saved name ----
sv_name <- here(out_path, "ame_sensitivity1.xlsx")
sv_name2 <- here(mdl_ame_path, "ame_main_sensitivity1.rds")
sv_name3 <- here(mdl_ame_path, "ame_fintech_sensitivity1.rds")
...
# Read models ----
mdl_account <- readRDS(here(mdl_path, "glm_account.rds"))
mdl_saving <- readRDS(here(mdl_path, "glm_saving.rds"))
mdl_credit <- readRDS(here(mdl_path, "glm_credit.rds"))
mdl_mobintpay <- readRDS(here(mdl_path, "glm_mobintpay.rds"))
mdl_mmoney <- readRDS(here(mdl_path, "glm_mmoney.rds"))
...
ame_main <- bind_rows(
  mod_tidy_num(tidy_ame(ame(mdl_account, vcov = "HC1"))) %>% 
    mutate(indicator = "Formal account"),
  mod_tidy_num(tidy_ame(ame(mdl_saving, vcov = "HC1"))) %>% 
    mutate(indicator = "Formal saving"),
  mod_tidy_num(tidy_ame(ame(mdl_credit, vcov = "HC1"))) %>% 
...
ame_fintech <- bind_rows(
  mod_tidy_num(tidy_ame_mi(ame_mi(mdl_mobintpay, vcov = "HC1"))) %>% 
    mutate(indicator = "Payment using Mobile"),
  mod_tidy_num(tidy_ame(ame(mdl_mmoney, vcov = "HC1"))) %>% 
